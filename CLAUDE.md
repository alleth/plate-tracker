...
In `routes/plates.js`, the named static routes **must stay above `/:id`**:
- `GET /dealers` — lists dealer accounts for the assignment dropdown
- `GET /stats/summary` — dashboard stat aggregates

Express matches routes in declaration order; moving these below `/:id` causes them to be swallowed by the parameterized handler.
...
`mv_file_number` is **not unique** — the constraint was intentionally dropped to allow multiple plates per MV file. `plate_number` has an application-level uniqueness check (space-normalized, case-insensitive).
